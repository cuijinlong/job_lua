...
--  获取request的参数
local subject_id = parameterUtil:getStrParam("subject_id","-1");
local structure_id = parameterUtil:getStrParam("structure_id","-1");
local sort_type = parameterUtil:getStrParam("sort_type","-1");
local sort_order = parameterUtil:getStrParam("sort_order","-1");
local page_size =  parameterUtil:getNubParam("page_size",0);
local page_number = parameterUtil:getNubParam("page_number",0);
--学生获得我的错题列表
local return_json = preparationModel:yxList(subject_id,structure_id,sort_type,sort_order,page_size,page_number);
cjson.encode_empty_table_as_object(false);
