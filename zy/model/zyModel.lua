...
function _ZyModel:save_zy_parse_paper(paper_list)
...
function _ZyModel:get_zy_answer_question_info(student_id,zy_id)
	...
	local answer_info,err = ssdb_db:hscan("homework_answer_"..StringUtil:trim(student_id).."_"..StringUtil:trim(zy_id),'','',200);
...
function _ZyModel:get_zy_zg_question_ids(zy_id)
    local dbUtil = require "zy.util.DbUtil";
    local cjson = require "cjson"
    local ssdb_db = dbUtil:getSSDb();
    local zg_list = {};
    local zg_question_info,err=ssdb_db:hget("homework_zy_content",zy_id)
    if  not  zg_question_info then
        say("{\"success\":false,\"info\":\"查询失败！\"}")
        return
    end
    if string.len(zg_question_info[1])>0 then
        local zycontent=zg_question_info[1]
        local zycon=cjson.decode(zycontent)
        if zycon.zg and (zycon.zg)[1] then
            zg_list = zycon.zg;
        end
    end
    --ngx.say(cjson.encode(zg_list));
    ssdb_db:set_keepalive(0,v_pool_size);
    return zg_list;
end
